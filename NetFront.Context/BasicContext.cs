...
public class BasicContext
{
    ...
    private void RunCore(FrontSettings set)
    {
        ...
        using var systemSocket = new TcpPubServer();
        using var clientSocket = new TcpPubServer();
        ...
        systemSocket.ReceiveReady += () =>
        {
            for (int i = 0; i < 200; i++)
            {
                if (!systemSocket.TryReceiveMultipartBytes(ref sysMsg)) break;
                if (!systemChannel.TryGetMsgType(sysMsg, out var msgType)) continue;
                switch (msgType)
                {
                    case (byte)MessageTypeEnum.PUBLIC_COMMAND:
                        if (!PublicCommandMessage.IsValid(sysMsg, out var commandType)) break;
                        switch (commandType)
                        {
                            case (byte)PublicCommandTypeEnum.SPUB:
                                if (sysMsg.Count == 2)
                                {
                                    if (PublicCommandMessage.TryGetTopic_SPUB(sysMsg[0], sysMsg[1], out byte[] topic))
                                    {
                                        if (PublicMessage.IsValidTopic(sysMsg[1]))
                                        {
                                            db.Set(topic, sysMsg[1]);
                                            clientChannel.SendPublicMessage(sysMsg[1]);
                                        }
                                    }
                                }
                                break;
                            ...
                            case (byte)PublicCommandTypeEnum.SET:
                                if (sysMsg.Count == 2)
                                {
                                    if (PublicCommandHeaderFrame.TryGetArgData_SET(sysMsg[0], out int offset, out int length))
                                    {
                                        db.Set(sysMsg[0].AsSpan(offset, length), sysMsg[1]);
                                    }
                                }
                                break;
                            default:
                    ...
                    case (byte)MessageTypeEnum.HEARTBEAT:
                        systemChannel.SendHeartbeatMessage(sysMsg[0]);
                        clientChannel.SendHeartbeatMessage(sysMsg[0]);
                        OnChannelStatisticsReceived(systemChannel.Statistics, clientChannel.Statistics);
                        break;
                    case (byte)MessageTypeEnum.FRONT_UNSUB:
                        if (FrontUnsubMessage.IsValid(sysMsg))
                        {
                            if (FrontUnsubMessage.TryGetTopic(sysMsg[0], out var topic))
                            {
                    ...
                    case (byte)MessageTypeEnum.USER_UNSUB:
                        if (UserUnsubMessage.IsValid(sysMsg))
                        {
                            if (UserUnsubMessage.TryGetTopic(sysMsg[0], out var topic))
                            {
                                systemChannel.Unsubscribe(topic);
                                if (UserSession.TryGetUserRspSession(topic, out var session))
                                {
                    ...
                    case (byte)MessageTypeEnum.HEARTBEAT_SUB:
                        if (HeartbeatSubMessage.IsValid(sysMsg))
                        {
                            systemChannel.Subscribe(HeartbeatMessage.Topic);
                        }
                        break;
                    case (byte)MessageTypeEnum.USER_REQ:
                        if (UserRequestMessage.IsValid(sysMsg))
                        {
                            var rspHeader = sysMsg[0].AsSpan();
                                ...
                                switch (functionID)
                                {
                                    case (byte)UserRequestFunctionEnum.LOGIN:
                                        ...
                                        if (ReqUserLogin.TryParse(reqData, out var reqLogin))
                                        {
                                            ...
                                            systemChannel.SendUserResponseMessage(sysMsg[0],
                                    ...
                                    case (byte)UserRequestFunctionEnum.SET_USER:
                                        if (um.SessionExists((byte)UserRoleEnum.SYSTEM, userSession))
                                        {
                                            if (sysMsg.Count == 2)
                                            {
                                                if (um.SetUser(sysMsg[1], out UserField user))
                                                {
                                                    systemChannel.SendUserResponseMessage(sysMsg[0],
                                                        RspInfoFrame.GetBytes(DateTime.UtcNow.Ticks, (int)ErrorCodeEnum.OK, "", true), []);
                                    ...
                                    case (byte)UserRequestFunctionEnum.SUB_ROUTE:
                                        if (um.SessionExists((byte)UserRoleEnum.SYSTEM, userSession))
                                        {
                                            if (RouteHeaderFrame.IsValidFunctionTopic(sysMsg[1]))
                                            {
                                                systemChannel.Subscribe(sysMsg[1]);
                                                systemChannel.SendUserResponseMessage(sysMsg[0],
                                                    RspInfoFrame.GetBytes(DateTime.UtcNow.Ticks, (int)ErrorCodeEnum.OK, "", true), []);
                                            }
                                        }
                                    ...
                                    case (byte)UserRequestFunctionEnum.RSP_ROUTE:
                                        if (um.SessionExists((byte)UserRoleEnum.SYSTEM, userSession))
                                        {
                                            if (sysMsg.Count == 4)
                                            {
                                                clientChannel.SendUserResponseMessage(sysMsg[1], sysMsg[2], sysMsg[3]);
                                                systemChannel.SendUserResponseMessage(sysMsg[0],
                                                    RspInfoFrame.GetBytes(DateTime.UtcNow.Ticks, (int)ErrorCodeEnum.OK, "", true), []);
                                            }
                                        }
                                    ...
                                    case (byte)UserRequestFunctionEnum.RTN_PRIVATE:
                                        if (um.SessionExists((byte)UserRoleEnum.SYSTEM, userSession))
                                        {
                                            if (PrivateMessage.IsValidUserTopic(sysMsg[1]))
                                            {
                                                clientChannel.SendPrivateMessage(sysMsg[1]);
                                                systemChannel.SendUserResponseMessage(sysMsg[0],
                                                    RspInfoFrame.GetBytes(DateTime.UtcNow.Ticks, (int)ErrorCodeEnum.OK, "", true), []);
                                            }
                                        }
        ...
        clientSocket.ReceiveReady += () =>
        {
            for (int i = 0; i < 200; i++)
            {
                if (!clientSocket.TryReceiveMultipartBytes(ref clientMsg)) break;
                if (!clientChannel.TryGetMsgType(clientMsg, out var msgType)) continue;
                switch (msgType)
                {
                    case (byte)MessageTypeEnum.FRONT_UNSUB:
                        if (FrontUnsubMessage.IsValid(clientMsg))
                        {
                            if (FrontUnsubMessage.TryGetTopic(clientMsg[0], out var topic))
                            {
                        ...
                        break;
                    case (byte)MessageTypeEnum.USER_UNSUB:
                        if (UserUnsubMessage.IsValid(clientMsg))
                        {
                            if (UserUnsubMessage.TryGetTopic(clientMsg[0], out byte[] topic))
                            {
                                clientChannel.Unsubscribe(topic);
                                if (UserSession.TryGetUserRspSession(topic, out var session))
                                {
                        ...
                        break;
                    case (byte)MessageTypeEnum.HEARTBEAT_SUB:
                        if (HeartbeatSubMessage.IsValid(clientMsg))
                        {
                            clientChannel.Subscribe(HeartbeatMessage.Topic);
                        }
                        break;
                    case (byte)MessageTypeEnum.USER_REQ:
                        if (UserRequestMessage.IsValid(clientMsg))
                        {
                            var rspHeader = clientMsg[0].AsSpan();
                            if (UserResponseHeaderFrame.TryConvert(rspHeader))
                            {
                                var functionID = UserResponseHeaderFrame.GetFunctionID(rspHeader);
                                var userSession = UserSession.GetUserRspSession(rspHeader);
                                ...
                                {
                                    case (byte)UserRequestFunctionEnum.LOGIN:
                                        var reqData = clientMsg[1].AsSpan();
                                        if (ReqUserLogin.TryParse(reqData, out var reqLogin))
                                        {
                                            var time = DateTime.UtcNow.Ticks;
                                            var userID = UserResponseHeaderFrame.GetUserID(rspHeader);
                                            ...
                                            clientChannel.SendUserResponseMessage(clientMsg[0],
                                                RspInfoFrame.GetBytes(time, errorCode, errorMsg, true), []);
                                            if (errorCode == 0)
                                            {
                                                OnSessionCreated(userID, userSession, time);
                                    ...
                                    case (byte)UserRequestFunctionEnum.SUB_PUBLIC:
                                        ...
                                        {
                                            if (PublicMessage.IsValidTopic(clientMsg[1]))
                                            {
                                                clientChannel.Subscribe(clientMsg[1]);
                                                if (db.TryGet(clientMsg[1], out var data))
                                                {
                                                    clientChannel.SendUserResponseMessage(clientMsg[0],
                                    ...
                                    case (byte)UserRequestFunctionEnum.SUB_PRIVATE:
                                        if (um.SessionExists((byte)UserRoleEnum.CLINET, userSession))
                                        {
                                            var userTopic = PrivateMessage.GetUserTopic(clientMsg[1], UserResponseHeaderFrame.GetBytesOfUserID(clientMsg[0]));
                                    ...
                                    case (byte)UserRequestFunctionEnum.REQ_ROUTE:
                                        if (um.SessionExists((byte)UserRoleEnum.CLINET, userSession))
                                        {
                                            if (RouteHeaderFrame.IsValidUserTopic(clientMsg[1]))
                                            {
                                                systemChannel.SendRouteMessage(clientMsg[1], clientMsg[0]);
                                            }
                                        }
                                        break;
                                    default:
    ...
    private static async void RunHeartbeat(FrontSettings set)
    {
        ...
        var timer = new PeriodicTimer(TimeSpan.FromMilliseconds(set.HEARTBEAT_INTERVAL));
